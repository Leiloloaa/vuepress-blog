...
解析用户 .vue 文件，然后根据用户调用的 createApp 去创建 虚拟dom，并调用 render 函数

第一步 用户调用了 createApp 创建 vnode，Vue 内部的 createApp 是调用了另外一个 createAppApi 函数，并且把 render 函数传递过去。**这样做的好处就是可以支持自定义渲染。**
...
createAppApi 返回的是一个函数 里面有 mount 方法

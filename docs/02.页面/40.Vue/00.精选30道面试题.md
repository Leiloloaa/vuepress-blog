...
- v-show 一定会渲染，只做简单的`display`切换
- v-if 是惰性的，它会根据初始条件渲染
...
Vuex 是一个专为 Vue.js 应用程序开发的**状态管理**模式。每一个 Vuex 应用的核心就是 store（仓库）。“store” 基本上就是一个容器，它包含着你的应用中大部分的状态 ( state )。
